...
options(echo=TRUE)
args <- commandArgs(trailingOnly = TRUE) # args <- c("outputs/20231016", "outputs/20231019", "outputs/20231030", 6, 'TRUE', 'TRUE')
...
filenames <- c(list.files(path=output.path, pattern = ".rds", full.names = TRUE), list.files(path=output.path2, pattern = ".rds", full.names = TRUE),list.files(path=output.path3, pattern = ".rds", full.names = TRUE))
...
for(f in filenames){
  ...
  for(j in 1:m){
    ...
    results[[f]][[j]] <- list("var"=var,"R"=R, "ncovars"=ncovars)
  }
}
...
results.df <- results.df %>%
  group_by(comparison,overlap.setting,gamma.setting,ncovars,J) %>% 
  mutate(oracle.var= mean(var[Estimator=="TMLE-multi. (GLM)"])) %>% 
  group_by(Estimator,comparison,overlap.setting,gamma.setting,ncovars,J) %>% 
  mutate(rel.var = mean((oracle.var+.Machine$double.eps)/(var+.Machine$double.eps)))
...
# reshape and plot
results.df$id <- with(results.df, paste(overlap.setting, gamma.setting, ncovars, J, sep = "_"))
results_long <- reshape2::melt(results.df[!colnames(results.df) %in% c("J","id","filename")], id.vars=c("Estimator","comparison","overlap.setting","gamma.setting","ncovars"))  # convert to long format
...
rel.var.df <- data.frame(results.df) %>% 
  select(c(rel.var,Estimator,overlap.setting,gamma.setting,ncovars)) %>% 
  group_by(Estimator,overlap.setting,gamma.setting,ncovars) %>% 
  mutate(rel.var = mean(rel.var)) %>%
  dplyr::slice(n()) %>%
  select(c(rel.var,Estimator,overlap.setting,gamma.setting,ncovars))

# 40 covariates
sim.results.rel.var.avg.40 <- ggplot(data=rel.var.df[rel.var.df$ncovars==40,],
...
# Get the ggplot grob
z.rel.var.avg <- ggplotGrob(sim.results.rel.var.avg.40)
...
ggsave(paste0("sim_results/static_simulation_rel_var_avg_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_covars_40_",covars40,"_covars_100_FALSE","_R_",results[[1]][[1]]$R,".png"),plot = z.rel.var.avg,scale=1.75)

# 100 covariates
sim.results.rel.var.avg.100 <- ggplot(data=rel.var.df[rel.var.df$ncovars==100,],
                                     aes(x=Estimator, y=rel.var, fill=forcats::fct_rev(Estimator)))  + geom_col() +
  facet_grid(overlap.setting ~  gamma.setting, scales = "free", labeller=labeller3)   + ylab("Average precision relative to TMLE-multi. (GLM) over all pairwise comparisons") +  
  scale_fill_manual(values= estimator.color) +
  scale_x_discrete(labels=NULL, limits = rev) +
...
# Get the ggplot grob
z.rel.var.avg <- ggplotGrob(sim.results.rel.var.avg.100)
...
ggsave(paste0("sim_results/static_simulation_rel_var_avg_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_covars_40_FALSE","_covars_100_",covars100,"_R_",results[[1]][[1]]$R,".png"),plot = z.rel.var.avg,scale=1.75)