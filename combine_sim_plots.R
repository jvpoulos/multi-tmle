...
filenames <- filenames[grep("tmle",filenames)]
...
for(f in filenames){
  ...
  if(isTRUE(grep("use_SL_FALSE",f)==1)){
    if(isTRUE(grep("tmle", f)==1)){
      estimator <- c("tmle_glm","tmle_glm_bin")
    ...
    if(isTRUE(grep("tmle", f)==1)){
      estimator <- setdiff(estimators[grep("tmle", estimators)],c("tmle_glm","tmle_glm_bin"))
    }
  }
...
results.df$Estimator <- c(rep("Multinomial (super learner)",length.out=length(c(unlist(CP[[1]])))), rep("Binomial (super learner)",length.out=length(c(unlist(CP[[2]])))),
                          rep("Multinomial (GLM)",length.out=length(c(unlist(CP[[1]])))), rep("Binomial (GLM)",length.out=length(c(unlist(CP[[2]])))))
...
# bias 
sim.results.bias <- ggplot(data=results_long[results_long$variable=="abs.bias" & results_long$Estimator!="Binomial (GLM)",],
  ...
  facet_grid(overlap.setting ~  gamma.setting, scales = "free", labeller=labeller3)  +  xlab(xlabel) + ylab("Absolute bias") +  ggtitle(paste0("Absolute bias (J=",J,", n=", n,")")) +
  scale_fill_discrete(name = "TMLE:") +
  scale_x_discrete(labels=x.labels,
                   limits = rev) +
  theme(legend.position="bottom") +   theme(plot.title = element_text(hjust = 0.5, family="serif", size=26)) +
  theme(axis.title=element_text(family="serif", size=26)) +
  theme(axis.text.y=element_text(family="serif", size=20)) +
  theme(axis.text.x=element_text(family="serif", size=10, angle = 0, vjust = 0.5, hjust=0.25)) +
  theme(legend.text=element_text(family="serif", size=16)) +
  theme(legend.title=element_text(family="serif", size=16)) +
  theme(strip.text.x = element_text(family="serif", size=20)) +
  theme(strip.text.y = element_text(family="serif", size=20)) +
...
ggsave(paste0("sim_results/static_simulation_bias_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.bias,scale=2)

# coverage
sim.results.coverage <- ggplot(data=results_long[results_long$variable=="CP" & results_long$Estimator!="Binomial (GLM)",],
  ...
  scale_colour_discrete(name = "TMLE:") +
  scale_x_discrete(labels=x.labels,
                   limits = rev) +
...
ggsave(paste0("sim_results/static_simulation_coverage_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.coverage,scale=2)

# CI width
sim.results.CI.width <- ggplot(data=results_long[results_long$variable=="CIW" & results_long$Estimator!="Binomial (GLM)",],
                               aes(x=comparison, y=value, fill=forcats::fct_rev(Estimator)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
...
if(J!=6 || n!=6000){
  sim.results.CI.width <- sim.results.CI.width + ggtitle(paste0("Confidence interval width (J=",J,", n=", n,")"))
}
...
ggsave(paste0("sim_results/static_simulation_ci_width_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.CI.width,scale=2)
...
sim.results.cp.avg <- ggplot(data=coverage.df[coverage.df$Estimator!="Binomial (GLM)",],
  ...
  theme(axis.title=element_text(family="serif", size=26)) +
  theme(axis.text.y=element_text(family="serif", size=20)) +
  theme(axis.text.x=element_text(family="serif", size=16, angle = 0, vjust = 0.5, hjust=0.25)) +
  theme(legend.text=element_text(family="serif", size=16)) +
  theme(legend.title=element_text(family="serif", size=16)) +
  theme(strip.text.x = element_text(family="serif", size=20)) +
  theme(strip.text.y = element_text(family="serif", size=20)) +
...
if(J!=6 || n!=6000){
  sim.results.cp.avg <- sim.results.cp.avg + ggtitle(paste0("Coverage probability (J=",J,", n=", n,")"))
}
...
ggsave(paste0("sim_results/static_simulation_cp_avg_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.cp.avg,scale=2)
...
sim.results.bias.avg <- ggplot(data=bias.df[bias.df$Estimator!="Binomial (GLM)",],
...
if(J!=6 || n!=6000){
  sim.results.bias.avg <- sim.results.bias.avg+ ggtitle(paste0("Bias (J=",J,", n=", n,")"))
}
...
ggsave(paste0("sim_results/static_simulation_bias_avg_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.bias.avg,scale=2)

## plot estimated and observed Ys and As, initial estimates, and ESS
...
for(f in filenames){
  ...
  if(any(estimator %in% estimators[grep("tmle", estimators)])){
    if(isTRUE(grep("use_SL_FALSE",f)==1)){
...
obs_outcome <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$obs_outcome)
obs_treatment <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$obs_treatment)
obs_treatment_n <- sapply(1:length(obs_treatment), function(i) obs_treatment[[i]]*n)
obs_covariates <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$obs_covariates)
true_taus <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$true_taus)
yinitial_tmle <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$yinitial_tmle)
ess_tmle <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$ess_tmle)
Ahat_tmle <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$Ahat_tmle)
yinitial_tmle_bin <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$yinitial_tmle_bin)
ess_tmle_bin <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$ess_tmle_bin)
Ahat_tmle_bin <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), function(f) results.AY[[f]]$Ahat_tmle_bin)
yinitial_tmle_glm <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_TRUE",filenames)]), function(f) results.AY[[f]]$yinitial_tmle_glm)
ess_tmle_glm <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_TRUE",filenames)]), function(f) results.AY[[f]]$ess_tmle_glm)
Ahat_tmle_glm <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_TRUE",filenames)]), function(f) results.AY[[f]]$Ahat_tmle_glm)
yinitial_tmle_glm_bin <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_TRUE",filenames)]), function(f) results.AY[[f]]$yinitial_tmle_glm_bin)
ess_tmle_glm_bin <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_TRUE",filenames)]), function(f) results.AY[[f]]$ess_tmle_glm_bin)
Ahat_tmle_glm_bin <- lapply(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_TRUE",filenames)]), function(f) results.AY[[f]]$Ahat_tmle_glm_bin)

# Create dataframe for plot
results.AY.df <- data.frame("Obs.A"=c(unlist(obs_treatment)),
                            "Obs.Y"=c(unlist(obs_outcome)),
                            "filename"=rep(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]), each=J*R)) # 54000
...
results.AY.preds.df <- data.frame("Y.initial"=c(unlist(yinitial_tmle),unlist(yinitial_tmle_bin),unlist(yinitial_tmle_glm),unlist(yinitial_tmle_glm_bin)),
                                  ...
                                  "Estimator"=c(rep("Multinomial (super learner)",length.out=length(unlist(Ahat_tmle))), rep("Binomial (super learner)",length.out=length(unlist(Ahat_tmle_bin))), rep("Multinomial (GLM)",length.out=length(unlist(Ahat_tmle_glm))), rep("Binomial (GLM)",length.out=length(unlist(Ahat_tmle_glm_bin)))),
                                  filename=c(unlist(sapply(1:length(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)])), function(i) rep(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)])[i], length.out=1000*J))))) # (1000*9*6) *5 = (54000)*5 = 270000

results.AY.df$J <- ifelse(J==3,3,6)
results.AY.preds.df$J <- ifelse(J==3,3,6)

results.AY.df$Treatment <- rep(rep(1:J, each=R), length(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)])))
results.AY.preds.df$Treatment <- rep(rep(rep(1:J, each=R), length(setdiff(filenames[grep("tmle", filenames)],filenames[grep("use_SL_FALSE",filenames)]))), n.estimators)
...
sim.results.A <- ggplot(data=results_AY_long[results_AY_long$variable=="Obs.A",],
  ...
  facet_grid(overlap.setting ~ ., scales = "fixed", labeller=labeller3)  +  xlab("Treatment level (j)") + ylab("Observed value") + ggtitle(paste0("Observed treatment probabilities (J=",J,", n=", n,")")) +
  scale_fill_manual(name = "Variable:", values = c("orange")) + 
  scale_x_discrete(labels=x.labels.AY,
                   limits = rev) +
  theme(legend.position="none") +   theme(plot.title = element_text(hjust = 0.5, family="serif", size=26)) +
  theme(axis.title=element_text(family="serif", size=26)) +
  theme(axis.text.y=element_text(family="serif", size=20)) +
...
ggsave(paste0("sim_results/static_simulation_obs_A_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.A,scale=2)

# observed outcomes
sim.results.Y <- ggplot(data=results_AY_long[results_AY_long$variable=="Obs.Y",],
                        aes(x=factor(Treatment), y=value,fill=forcats::fct_rev(variable)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
  facet_grid(overlap.setting ~  gamma.setting, scales = "fixed", labeller=labeller3)  +  xlab("Treatment level (j)") + ylab("Observed value") + ggtitle(paste0("Observed outcomes under each treatment level (J=",J,", n=", n,")")) +
  scale_fill_manual(name = "Variable:", values = c("orange")) + 
  scale_x_discrete(labels=x.labels.AY,
                   limits = rev) +
  theme(legend.position="none") +   theme(plot.title = element_text(hjust = 0.5, family="serif", size=26)) +
...
ggsave(paste0("sim_results/static_simulation_Y_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.Y,scale=2)

# treatment estimates
sim.results.A.mean <- ggplot(data=results_AY_preds_long[results_AY_preds_long$variable=="A.mean" & results_AY_preds_long$Estimator!="Binomial (GLM)",],  
  ...
  facet_grid(overlap.setting ~ gamma.setting, scales = "fixed", labeller=labeller3)  +  xlab("Treatment level (j)") + ylab("Estimated value") + ggtitle(paste0("Estimated treatment probabilities (J=",J,", n=", n,")")) +
  scale_fill_discrete(name = "TMLE:") +
  scale_x_discrete(labels=x.labels.AY,
                   limits = rev) +
  theme(legend.position="bottom") +   theme(plot.title = element_text(hjust = 0.5, family="serif", size=26)) +
...
ggsave(paste0("sim_results/static_simulation_est_A_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.A.mean,scale=2)

# treatment estimates -diff
sim.results.A.mean.diff <- ggplot(data=results_AY_preds_long[results_AY_preds_long$variable=="A.mean.diff" & results_AY_preds_long$Estimator!="Binomial (GLM)",],  
                             aes(x=factor(Treatment), y=value,fill=forcats::fct_rev(Estimator)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
...
ggsave(paste0("sim_results/static_simulation_est_A_diff_estimand_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.A.mean.diff,scale=2)

# initial outcome estimates
sim.results.Y.initial <- ggplot(data=results_AY_preds_long[results_AY_preds_long$variable=="Y.initial" & results_AY_preds_long$Estimator!="Binomial (GLM)",], 
                             aes(x=factor(Treatment), y=value,fill=forcats::fct_rev(Estimator)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
  facet_grid(overlap.setting ~ gamma.setting, scales = "fixed", labeller=labeller3)  +  xlab("Treatment level (j)") + ylab("Estimated value") + ggtitle(paste0("Initial outcome estimate (J=",J,", n=", n,")")) +
  scale_fill_discrete(name = "TMLE:") +
  scale_x_discrete(labels=x.labels.AY,
                   limits = rev) +
  theme(legend.position="bottom") +   theme(plot.title = element_text(hjust = 0.5, family="serif", size=26)) +
...
ggsave(paste0("sim_results/static_simulation_est_Y_initial_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.Y.initial,scale=2)

# initial outcome estimates
sim.results.Y.initial.diff <- ggplot(data=results_AY_preds_long[results_AY_preds_long$variable=="Y.initial.diff" & results_AY_preds_long$Estimator!="Binomial (GLM)",], 
                                aes(x=factor(Treatment), y=value,fill=forcats::fct_rev(Estimator)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
...
ggsave(paste0("sim_results/static_simulation_est_Y_initial_diff_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.Y.initial.diff,scale=2)

# ESS
sim.results.ESS <- ggplot(data=results_AY_preds_long[results_AY_preds_long$variable=="ESS" & results_AY_preds_long$Estimator!="Binomial (GLM)",],
                             aes(x=factor(Treatment), y=value,fill=forcats::fct_rev(Estimator)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
  facet_grid(overlap.setting ~ gamma.setting, scales = "fixed", labeller=labeller3)  +  xlab("Treatment level (j)") + ylab("ESS") + ggtitle(paste0("Effective sample size (J=",J,", n=", n,")")) +
  scale_fill_discrete(name = "TMLE:") +
  scale_x_discrete(labels=x.labels.AY,
                   limits = rev) +
  theme(legend.position="bottom") +   theme(plot.title = element_text(hjust = 0.5, family="serif", size=26)) +
...
ggsave(paste0("sim_results/static_simulation_est_ESS_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.ESS,scale=2)

# ESS_ratio
sim.results.ESS_ratio <- ggplot(data=results_AY_preds_long[results_AY_preds_long$variable=="ESS_ratio" & results_AY_preds_long$Estimator!="Binomial (GLM)",],
                          aes(x=factor(Treatment), y=value,fill=forcats::fct_rev(Estimator)))  + geom_boxplot(outlier.alpha = 0.3,outlier.size = 1, outlier.stroke = 0.1, lwd=0.25) +
  facet_grid(overlap.setting ~ gamma.setting, scales = "fixed", labeller=labeller3)  +  xlab("Treatment level (j)") + ylab(TeX(paste0('$ESS_j/n_j$'))) + ggtitle(paste0("Effective sample size ratio (J=",J,", n=", n,")")) +
  scale_fill_discrete(name = "TMLE:") +
  scale_x_discrete(labels=x.labels.AY,
                   limits = rev) +
...
ggsave(paste0("sim_results/static_simulation_est_ESS_ratio_",estimand,"_J_",J,"_n_",n,"_outcome_",outcome.type,"_R_",results[[1]][[1]]$R,".png"),plot = z.ESS_ratio,scale=2)